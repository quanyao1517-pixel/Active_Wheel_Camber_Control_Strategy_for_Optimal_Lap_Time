...
[Fxflc,Fyflc] = tire_model(Fzfl,SA_flreal,SLflreal,-Cam_flreal);
[Fxfrc,Fyfrc] = tire_model(Fzfr,SA_frreal,SLfrreal,Cam_frreal);
[Fxrlc,Fyrlc] = tire_model(Fzrl,SA_rlreal,SLrlreal,-Cam_rlreal);
[Fxrrc,Fyrrc] = tire_model(Fzrr,SA_rrreal,SLrrreal,Cam_rrreal);
...
pianyi=0.08;
for k = 1:(N)
    g = [g;(U(28,k)*camberuni-control0704.U(28,k)-pianyi/180*3.1415926)];

end


for k = 1:(N)
    g = [g;((U(29,k)*camberuni-control0704.U(29,k)-pianyi/180*3.1415926))];
end

for k = 1:(N)
    g = [g;((U(30,k)*camberuni-control0704.U(30,k)-pianyi/180*3.1415926))];
end
    
for k = 1:(N)
    g = [g;((U(31,k)*camberuni-control0704.U(31,k))-pianyi/180*3.1415926)];
end
...
args = struct;
args.lbg = zeros(nx*(N+1) + N*35,1);  
args.ubg = zeros(nx*(N+1) + N*35,1);
...
%后轮主动轮荷转移限制
args.lbg(nx*(N+1)+N*31+1:nx*(N+1)+N*35) = -0;  
args.ubg(nx*(N+1)+N*31+1:nx*(N+1)+N*35) = 0;
...
% %车辆参数限制
% args.lbx(nu*N+nx*(N+1)+1) = p_init(1)-whellbasechange;
% args.ubx(nu*N+nx*(N+1)+1) = p_init(1)+whellbasechange;
% args.lbx(nu*N+nx*(N+1)+2) = p_init(2)-whellbasechange;
% args.ubx(nu*N+nx*(N+1)+2) = p_init(2)+whellbasechange;
% args.lbx(nu*N+nx*(N+1)+3) = -fronttoeangle;
% args.ubx(nu*N+nx*(N+1)+3) = fronttoeangle;
% args.lbx(nu*N+nx*(N+1)+4) = -reartoeangel;
% args.ubx(nu*N+nx*(N+1)+4) = reartoeangel;
...
for k = 1:N+1
    ...
    res.littletime(k)=((step_length-res.X(1,k)*Track.curv(k))/(res.X(3,k)*cos(res.X(2,k))-res.X(4,k)*sin(res.X(2,k))));
end
...
% end
for i=1:N
 res.camber(1,i)=res.U(28,i);
 res.camber(2,i)=res.U(29,i);
 res.camber(3,i)=res.U(30,i);
 res.camber(4,i)=res.U(31,i);
end
res.frontsteering=res.U(1,:)*180/pi;
res.steeringspeed(1)=(res.frontsteering(1)-res.frontsteering(N))/res.littletime(i);
for i=1:N-1
res.steeringspeed(i+1)=(res.frontsteering(i+1)-res.frontsteering(i))/res.littletime(i);
end

res.camfl=res.camber(1,:)*180/pi;
res.camfr=res.camber(2,:)*180/pi;
res.camrl=res.camber(3,:)*180/pi;
res.camrr=res.camber(4,:)*180/pi;


res.camflspeed(1)=(res.camfl(1)-res.camfl(N))/res.littletime(i);
res.camfrspeed(1)=(res.camfr(1)-res.camfr(N))/res.littletime(i);
res.camrlspeed(1)=(res.camrl(1)-res.camrl(N))/res.littletime(i);
res.camrrspeed(1)=(res.camrr(1)-res.camrr(N))/res.littletime(i);


for i=1:N-1
res.camflspeed(i+1)=(res.camfl(i+1)-res.camfl(i))/res.littletime(i);
res.camfrspeed(i+1)=(res.camfr(i+1)-res.camfr(i))/res.littletime(i);
res.camrlspeed(i+1)=(res.camrl(i+1)-res.camrl(i))/res.littletime(i);
res.camrrspeed(i+1)=(res.camrr(i+1)-res.camrr(i))/res.littletime(i);
end
...
args.lbg(nx*(N+1)+N*31+1:nx*(N+1)+N*35) = -0;  
args.ubg(nx*(N+1)+N*31+1:nx*(N+1)+N*35) = 0;
lag.clam_fl=full(sol.lam_g(nx*(N+1)+N*31+1:nx*(N+1)+N*32));
lag.clam_fr=full(sol.lam_g(nx*(N+1)+N*32+1:nx*(N+1)+N*33));
lag.clam_rl=full(sol.lam_g(nx*(N+1)+N*33+1:nx*(N+1)+N*34));
lag.clam_rr=full(sol.lam_g(nx*(N+1)+N*34+1:nx*(N+1)+N*35));


% 格式化文件名，%03d 代表3位数字，不足补0
filename = sprintf('lag00%01d.mat',pianyi*100);
save(filename, 'lag');
...
scatter(lateral_acc,camber,  50, lag.clam_fl, 'filled'); % 点大小50，颜色由纵向加速度决定
colormap('parula'); % 使用parula配色方案
colorbar; % 添加颜色条
c = colorbar;
c.Label.String = '拉格朗日乘子'; % 颜色条标签
c.Label.FontSize = 12;
