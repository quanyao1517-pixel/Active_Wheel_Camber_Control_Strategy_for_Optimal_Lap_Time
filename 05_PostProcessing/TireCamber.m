...
plot(Track.S(1:length(res_pass.U(1,:))), res_ctrl.camber(1,:)*180/3.1415926, 'r-.', 'Linewidth', 4); hold on;
plot(Track.S(1:length(res_pass.U(1,:))), res_pass.camber(1,:)*180/3.1415926, 'b', 'Linewidth', 4);
ylabel('左前(deg)', 'FontName', 'SimSun', 'FontSize', font_size);
...
plot(Track.S(1:length(res_pass.U(1,:))), res_ctrl.camber(2,:)*180/3.1415926, 'r-.', 'Linewidth', 4); hold on;
plot(Track.S(1:length(res_pass.U(1,:))), res_pass.camber(2,:)*180/3.1415926, 'b', 'Linewidth', 4);
ylabel('右前(deg)', 'FontName', 'SimSun', 'FontSize', font_size);
...
plot(Track.S(1:length(res_pass.U(1,:))), res_ctrl.camber(3,:)*180/3.1415926, 'r-.', 'Linewidth', 4); hold on;
plot(Track.S(1:length(res_pass.U(1,:))), res_pass.camber(3,:)*180/3.1415926, 'b', 'Linewidth', 4);
ylabel('左后(deg)', 'FontName', 'SimSun', 'FontSize', font_size);
set(ax3, 'Box', 'off', 'Linewidth', 1.2, 'FontSize', font_size, 'FontWeight', 'bold', ...
    'TickDir', 'out', 'FontName', 'Times New Roman', 'XTickLabel', []);
xlim(ax3, x_range);
ylim(ax3, y_range);
set(ax3, 'YTick', y_ticks);
% 明确设置刻度标签的字体
set(gca, 'FontName', '宋体', 'FontSize', font_size, 'LineWidth', 4);
% 子图4
ax4 = axes('Position', subplot_positions(4,:));
plot(Track.S(1:length(res_pass.U(1,:))), res_ctrl.camber(4,:)*180/3.1415926, 'r-.', 'Linewidth', 4); hold on;
plot(Track.S(1:length(res_pass.U(1,:))), res_pass.camber(4,:)*180/3.1415926, 'b', 'Linewidth', 4);
xlabel('赛车行驶距离 (m)', 'FontName', 'SimSun', 'FontSize', font_size);
ylabel('右后(deg)', 'FontName', 'SimSun', 'FontSize', font_size);
